...
const REPO_ROOT = "C:/Users/User/projects/qradar-soc-operations-handbook";
...
// "### Section X — Name" subheadings — same shape as the DEH/SOC-Manager indexes.
// Unlike those two sibling books, this book's three appendices (A1-A3) are explicitly
// "planned; not yet authored" (BOOK-INDEX.md's own Appendix Table note) — only the 22
// chapters are drafted and reviewed. Stop parsing at "## Appendix Table" so the build
// doesn't emit three MISSING FILE placeholder pages for content that doesn't exist yet.
function parseIndex() {
  ...
  for (const raw of lines) {
    ...
    if (line === "## Appendix Table") {
      break; // appendices not yet authored — see comment above
    }
    ...
    if (line.startsWith("|")) {
      ...
      // split, then restore, so an escaped pipe never shifts a later column boundary.
...
// Six content tags per STYLE-GUIDE.md §7. CONCEPT/ANALYST/THREAT HUNTER/SOC MANAGEMENT
// are unchanged from the Detection Engineering Handbook; RULE ENGINEER and PLATFORM
// ENGINEER are this book's renames of DEH's DETECTION ENGINEER and ENGINEERING tags
// (§7's rationale: same role split, narrowed to one platform) - kept on the same
// color lineage so a reader moving between NESHBOY titles sees a consistent palette.
const TAG_COLORS = {
  CONCEPT: "tag-concept",
  ANALYST: "tag-analyst",
  "RULE ENGINEER": "tag-rule-engineer",
...
function main() {
  ...
  * { box-sizing: border-box; }
  ...
  .tag-rule-engineer { background: var(--blue-tint); color: #164a8a; border: 0.5pt solid var(--blue); }
...
<head>
<meta charset="utf-8">
<title>QRadar SOC Operations</title>
<style>${css}</style>
</head>
<body>
<section class="cover">
  <div class="kicker">NESHBOY SOC Professional Library</div>
  <h1>QRADAR SOC OPERATIONS</h1>
  <div class="accent-rule"></div>
  <h2>Offense Management, Rule Engineering, DSM Onboarding, and Platform Operations — Running QRadar Beyond the Query Language</h2>
