...
- **Reference Set / Reference Map / Reference Table / Reference Sequence names** are always written with the `RS-` prefix (or the equivalent prefix your example has established for a Map/Table/Sequence) in inline code: `` `RS-Allowlisted-LSASS-Tools` ``. State the reference-data *type* (Set vs. Map vs. Table vs. Sequence) at first mention in a part — they are not interchangeable, and Part 10 covers exactly where each type is the right choice.
